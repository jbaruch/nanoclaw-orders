...
description: "Cadence wrapper that runs check-orders on its own schedule: fetch order emails, update the orders table in messages.db, flag anomalies, emit an observable-silence cursor marker. Peeled off the nightly-external-sync bundle (jbaruch/nanoclaw#581) so orders run in a bounded container. Triggers: 'order sync', 'sync orders', 'nightly order sync', 'check my orders nightly'."
...
`Skill(skill: "tessl__check-orders")` — fetch order emails, update the `orders` table in `messages.db`, flag anomalies. The inner skill reports flagged items via `mcp__nanoclaw__send_message` and is otherwise silent.
