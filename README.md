...
- `scripts/flag-anomalies.py` — applies the anomaly predicates (owns the statuses, per-status age cutoffs, and the supersession test that keeps a split order's stale row quiet); flags the supplied stuck ids
